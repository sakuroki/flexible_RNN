...
from pyrl          import inactivation # added by sk 2017/2/28
...
def do(action, args, config):
    ...
    #=====================================================================================
    elif 'inactivation' in action: # added by sk 2017/2/28 for inactivation experiment
        n_neuron = 100
        try:
            trials_per_condition = int(args[0])
        except:
        ...
        try:
            ...
            w_inact_dep = int(n_neuron / w_inact_bin)

        model = config['model']
        # modified by sk 2017/2/28
        pgi    = model.get_pgi(config['savefile'], config['seed'], config['dt'])
        ...
        for n in xrange(n_trials):
            ...
            trials.append(task.get_condition(pgi.rng, pgi.dt, context))

        # added by sk 2017/2/28: sort with Wrec_diff
        import cPickle as pickle
        fn = 'path\\to\\initial\\model'
        with open(fn, 'rb') as f:
            data = pickle.load(f)
        policy_Wrec_init = data['best_policy_params']['Wrec'].T
        #baseline_Wrec_init = data['best_baseline_params']['Wrec'].T

        fn = 'path\\to\\last\\model'
        with open(fn, 'rb') as f:
            data = pickle.load(f)
        policy_Wrec_last = data['best_policy_params']['Wrec'].T
        #baseline_Wrec_last = data['best_baseline_params']['Wrec'].T

        policy_Wrec_diff =  []
        if 'diff' in action:
            policy_Wrec_diff = policy_Wrec_last - policy_Wrec_init
        elif 'last' in action:
            policy_Wrec_diff = policy_Wrec_last
        #baseline_Wrec_diff = baseline_Wrec_last - baseline_Wrec_init

        Neu_rec_diff = np.mean(np.abs(policy_Wrec_diff), axis = 1)
        sorted_val =  np.sort(Neu_rec_diff)[::-1]
        sorted_id = np.argsort(Neu_rec_diff)[::-1]

        #Neu_rec_diff_base = np.mean(np.abs(baseline_Wrec_diff), axis = 1)
        #sorted_val_base =  np.sort(Neu_rec_diff)[::-1]
        #sorted_id_base = np.argsort(Neu_rec_diff)[::-1]

        for i in xrange(w_inact_dep):
            id_inact = sorted_id[:(i+1)*w_inact_bin]
            print(id_inact)
            inactivation.run(action, trials, pgi, config['trialspath'], id_inact)
        ...
        sorted_id = range(n_neuron)
        np.random.seed(config['seed'])
        np.random.shuffle(sorted_id)
        sorted_val = Neu_rec_diff[sorted_id]
        ...
        inactivation.run(action, trials, pgi, config['trialspath']) # for full condition

        print ('finished: inactivation')
