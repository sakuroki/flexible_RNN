...
from pycog          import fittools, RNN, tasktools, inactivation
...
def get_trialsfile_inact(p,tag):
    return join(p['trialspath'], p['name'] + '_trials_'+tag+'.pkl')
...
#=========================================================================================
def run_trials_inact(p, id_inact, args):  # added by sk for inactivation experiment
    # Model
    m = p['model']
    ...
    # RNN
    rng = np.random.RandomState(p['seed'])
    rnn = inactivation.RNN_inact(p['savefile'], {'dt': p['dt']}, verbose=False)
    ...
    try:
        for i in xrange(ntrials):
            ...
            info = rnn.run(inputs=(trial_func, trial_args), id_inact=id_inact, rng=rng)
    ...
    # Save all
    filename = get_trialsfile_inact(p,str_ids)
    with open(filename, 'wb') as f:
        pickle.dump(trials, f, pickle.HIGHEST_PROTOCOL)
    size = os.path.getsize(filename)*1e-9
    print("[ {}.run_trials ] Trials saved to {} ({:.1f} GB)".format(THIS, filename, size))
