...
# modified by Satoshi Kuroki, 2017
...
train :
  ...
  def train(self, gradient_dataset, cg_dataset, initial_lambda=0.1, mu=0.03, global_backtracking=False, preconditioner=False,
             max_cg_iterations=250, num_updates=100, validation=None, validation_frequency=1, patience=numpy.inf, save_progress=None,
             logging=None):
  ...
  save_progress: string or None
      ...
      training.
  logging: string or None (added by sk to logging the learning process)
      logging parameters of the training process a this directory after each update.'''
    ...
    # added by sk to logging the learning process
    if isinstance(logging, str):
        if not os.path.exists(logging):
            os.mkdir(logging)
        logging_file = logging + 'ref.pkl'
        if not os.path.exists(logging_file):
          log_ref_int = ['initial_lambda','mu','global_backtracking','preconditioner','max_cg_iterations','num_updates',
               'validation_frequency','patience']
          log_ref = ['iteration','p','costs_train','costs_valid','lamda','after_cost','flat_delta','backtracking',
                 'num_cg_iterations','delta_cost','before_cost','rho','cg_last_x','best_u']
          log = log_ref_int, log_ref
          cPickle.dump(log, file(logging_file, 'wb'), cPickle.HIGHEST_PROTOCOL)
          logging_file = logging + 'initial_condition.pkl'
          log = initial_lambda, self.mu, self.global_backtracking, self.preconditioner, self.max_cg_iterations, \
                num_updates, validation_frequency, patience
          cPickle.dump(log, file(logging_file, 'wb'), cPickle.HIGHEST_PROTOCOL)
          logging_file = logging + 'model_0.pkl'
          log = 0, [i.get_value().copy() for i in self.p], float('nan'), float('nan'), self.lambda_,float('nan'), float('nan'), \
    ...
    try:
      for u in xrange(first_iteration, 1 + num_updates):
        ...
        costs_train = [] # edit by sk cost -> cost_train
        for inputs in gradient_dataset.iterate(update=True):
          ...
          costs_train.append(result[len(self.p):])

        print 'cost=', numpy.mean(costs_train, axis=0),
        ...
        costs_valid = [] # added by sk
        if validation is not None and u % validation_frequency == 0:
          if hasattr(validation, 'iterate'):
            costs_valid = numpy.mean([self.f_cost(*i) for i in validation.iterate()], axis=0)
          elif callable(validation):
            costs_valid = validation()
          print 'validation=', costs_valid,
          if costs_valid[0] < best[1]:
            best = u, costs_valid[0], [i.get_value().copy() for i in self.p]
            print '*NEW BEST',
          # added by sk to logging the learning process
          if isinstance(logging, str):
              logging_file = logging + 'model_' + str(u) + '.pkl'
              log = u, [i.get_value().copy() for i in self.p], costs_train, costs_valid, self.lambda_,after_cost, flat_delta, \
                  backtracking, num_cg_iterations, delta_cost, before_cost, rho, self.cg_last_x, best[0]
              cPickle.dump(log, file(logging_file, 'wb'), cPickle.HIGHEST_PROTOCOL)
    ...
    return best # edited by sk to return all information about the best


class SequenceDataset:
  '''Slices, shuffles and manages a small dataset for the HF optimizer.'''

  def __init__(self, data, batch_size, number_batches, minimum_size=10, seed=0):
