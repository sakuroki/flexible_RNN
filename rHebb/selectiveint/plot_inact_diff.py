...
Script for collect and visualize inactivation experiment.
"""
...
data_dir = 'path\\to\\task_peforming_results\\directory\\'
os.chdir(data_dir)
...
if not os.path.isfile(data_fn):
    ...
    # make pandas dataframe from the behavior results data
    ...
    Jdata_dir = 'path\\to\\directory\\which\\has\\weight_value_file\\'
    n_neuron = 200
    ...
    for RNGSEED in range(1,21):
        ...
        # read initial model
        fn = Jdata_dir + 'J_RNGSEED'+str(RNGSEED)+'_0.txt'
        Wrec_init = np.zeros((n_neuron, n_neuron))
        f = open(fn, 'r')
        reader = csv.reader(f, delimiter=" ")
        for i, row in enumerate(reader):
            roww = [float(r) for r in row if r != '']
        ...
        # calc the wieht difference between before and after learning
        Wrec_diff = Wrec_last - Wrec_init
        Neu_rec_diff = np.mean(np.abs(Wrec_diff), axis = 1)

        # descend sort and exlude the output and constant input units
        ...
        # ascend sort and exlude the output and constant input units
        ...
        # shuffle sort and exlude the output and constant input units
        ...
        # add sort tag to pandas dataframe
...
fig_name = 'result_plot_inact.png'
