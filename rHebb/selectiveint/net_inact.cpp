// Modified by Satoshi Kuroki, 2017 for inactivation experiment
...
int main(int argc, char* argv[])
{
    ...
    // added by sk 2017/2/21 for inactivation setting
    int NINACT = 0;
    int POSINACT = 0;
    ...
    if (argc > 1)
       for (int nn=1; nn < argc; nn++)
       {
           ...
           // added by sk 2017/2/21 to save inactivated unit IDs
           if (strcmp(argv[nn], "IDINACT") == 0) {
             NINACT = argc-nn-1;
             POSINACT = nn;
           }
       }

       // added by sk 2017/2/21 to memorize inactivated unit IDs
       int IDINACT[NINACT] = {};
         if(NINACT != 0){
           for (int nnn=0; nnn < NINACT; nnn++){
             IDINACT[nnn] = atoi(argv[POSINACT+nnn+1]);
           }
         }
    ...
    // added by sk, simplified the SUFFIX because it cannot save file  with original filenames.
    string SUFFIX = "_RNGSEED" + to_string(RNGSEED);
    cout << SUFFIX << endl;
...
int trialtype;
    ...
    for (int numtrial=0; numtrial < NBTRIALS; numtrial++)
    {
        ...
        // added by sk 2017/2/21 for initial unit inactivation
        if(NINACT != 0){
          if (PHASE == TESTING){
              for (int nnn=0; nnn < NINACT; nnn++){
                x(IDINACT[nnn]) = 0.0;
        ...
        */
        ...
        for (int numiter=0; numiter < TRIALTIME;  numiter++)
        {
            ...
            // added by sk 2017/2/21 unit inactivation
            if(NINACT != 0){
              if (PHASE == TESTING){
                  for (int nnn=0; nnn < NINACT; nnn++){
                    x(IDINACT[nnn]) = 0.0;
       ...
       int EVALTIME = 200;
        ...
        if (PHASE == LEARNING)
        {
            ...
            // added by sk 2017/2/21 to save learning process
            std::string str_inact = "_idinact_n" + std::to_string(NINACT);
            if(NINACT != 0){
              cout << " inact: ";
              for (int nnn=0; nnn < NINACT; nnn++){
                if(nnn < 5){
                  str_inact = str_inact + "_" + std::to_string(IDINACT[nnn]);
                }
                cout << " " << IDINACT[nnn];
              }
              cout << endl;
            }else{
              str_inact = str_inact + "_full";
              cout << " full" << endl;
            }

            //myfile.open("errs_type"+std::to_string(trialtype)+"_bias1_"+ std::to_string(BIAS1)+ "_bias2_"+ std::to_string(BIAS2)+ "_"+std::to_string(int(numtrial/ NBTRIALTYPES))+str_inact+".txt", ios::trunc | ios::out);  myfile << endl << meanerr << endl; myfile.close();
            //myfile.open("rs_long_type"+std::to_string(trialtype)+"_bias1_"+ std::to_string(BIAS1)+ "_bias2_"+ std::to_string(BIAS2)+ "_"+std::to_string(int(numtrial/ NBTRIALTYPES))+str_inact+".txt", ios::trunc | ios::out);  myfile << endl << rs.transpose() << endl; myfile.close();
            myfile.open("lastr_type"+std::to_string(trialtype)+"_bias1_"+ std::to_string(BIAS1) + "_bias2_"+ std::to_string(BIAS2)+ "_"+std::to_string(int(numtrial/ NBTRIALTYPES)) + SUFFIX + str_inact + ".txt", ios::trunc | ios::out);  myfile << endl << r(0) << endl; myfile.close();
        }
