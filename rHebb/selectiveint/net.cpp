...
int trialtype;
    ...
    // add by sk 2017/3/27 for uniform distribution
    //J.setRandom();
    //J = 0.1 * J;
       ...
       int EVALTIME = 200;
        ...
        if (PHASE == LEARNING)
        {
            ...
            if ((numtrial % 10000 == 0) || (numtrial == NBTRIALS - 1))
            {
                ...
                myfile.open("J" + SUFFIX + "_" + std::to_string(numtrial) + ".txt", ios::trunc | ios::out);  myfile << J << endl; myfile.close();
                saveWeights(J, "J" + SUFFIX + "_" + std::to_string(numtrial)+".dat");
